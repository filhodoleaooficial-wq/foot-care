...
serve(async (req) => {
  ...
  try {
    ...
    const hasPhone = phone && normalizePhone(phone).length >= 8;
    ...
    if (existing) {
      ...
      // Always update phone if user provided one (allows changing number)
      if (hasPhone) {
        updates.phone = phone;
        clientPhone = phone;
      }
      ...
      const { data: inserted, error } = await supabase
        .from("app_clients")
        .insert({ email, phone: hasPhone ? phone : null, age: 0, gender: "", user_id: userId || null })
        .select("id")
        .single();
